        ...
        <div class="card-body">
          ...
          <ul type='none'>
            <?php for ($i = 0; $i < count($gambar); $i++) : ?>
              <li><img src="<?= base_url(); ?>/assets/img/spbu/<?= $gambar[$i]; ?>" class="img-fluid"></li>
            <?php endfor; ?>
          </ul>
          <ul>
            <?php foreach ($web as $list) : ?>
              <li><?php echo $list; ?></li>
            <?php endforeach; ?>
          </ul>
          <hr>
          ...
          <hr>
          <ul>
            <?php foreach ($file as $list) : ?>
              <li><?php echo $list; ?></li>
            <?php endforeach; ?>
          </ul>
          <ul type='none'>
            <?php for ($i = 0; $i < count($mobile); $i++) : ?>
              <li><img src="<?= base_url(); ?>/assets/img/spbu/<?= $mobile[$i]; ?>" class="img-fluid"></li>
            <?php endfor; ?>
          </ul>
          <?php $list = array_keys($mode); ?>
          <ul>
            <?php for ($i = 0; $i < count($mode); $i++) : ?>
              <li>
                <?php echo $list[$i]; ?>
                <ul>
                  <?php foreach ($mode[$list[$i]] as $key) : ?>
                    <?php if ($key != "-") : ?>
                      <li>
                        <?php echo $key ?>
                      </li>
          ...
          <hr>
          <?php $list = array_keys($standalone); ?>
          <ul>
            <?php for ($i = 0; $i < count($standalone); $i++) : ?>
              <li>
                <?php echo $list[$i]; ?>
                <ul>
                  <?php foreach ($standalone[$list[$i]] as $key) : ?>
                    <?php if ($key != "-") : ?>
                      <li>
                        <?php echo $key ?>
                      </li>
