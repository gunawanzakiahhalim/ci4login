    ...
    <div class="tab-pane fade" id="nav-todolist" role="tabpanel" aria-labelledby="nav-todolist-tab">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <p><img src="<?= base_url(); ?>/assets/img/spbu/<?= $gambar; ?>" class="img-fluid" alt="startservice"></p>
          <?php $list = array_keys($gsms); ?>
          <ul>
            <?php for ($i = 0; $i < count($gsms); $i++) : ?>
              <li>
                <?php echo $list[$i]; ?>
                <ol>
                  <?php foreach ($gsms[$list[$i]] as $key) : ?>
                    <?php if ($key != "-") : ?>
                      <li>
                        <?php echo $key ?>
                      </li>
          ...
          <ul type='none'>
            <?php for ($i = 0; $i < count($komunikasi); $i++) : ?>
              <li><img src="<?= base_url(); ?>/assets/img/spbu/<?= $komunikasi[$i]; ?>" class="img-fluid"></li>
            <?php endfor; ?>
          </ul>
        </div>
      </div>
